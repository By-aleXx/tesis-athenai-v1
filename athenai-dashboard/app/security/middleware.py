...
# ========================================
# PREFIJOS DEL SERVIDOR PROTEGIDO (solo estos se registran)
# ========================================
# El logger de tráfico solo registra peticiones al servidor/aplicación
# protegida por el WAF. Las llamadas internas del dashboard de gestión
# NO son tráfico externo y no deben aparecer en los logs de tráfico.
MONITORED_PREFIXES = [
    '/api/victim/',       # Endpoints de la aplicación víctima protegida
    '/api/protected/',    # Endpoints externos protegidos (si se añaden)
]
...
class TrafficLoggingMiddleware:
    ...
    def after_request(self, response):
        ...
        """
        # Solo registrar tráfico al servidor protegido (whitelist de prefijos).
        # Las llamadas de gestión del dashboard (/api/stats, /api/alerts,
        # /api/auth/*, etc.) no son tráfico externo y no deben aparecer
        # en los logs de tráfico monitoreados.
        if not any(request.path.startswith(prefix) for prefix in MONITORED_PREFIXES):
