...
try:
    ...
    USE_CONFIG = False
    logger.warning("config.py no encontrado, usando variables de entorno directamente")
...
class BackupService:
    def __init__(self, endpoint_url=None, backup_bucket='athenai-backups'):
        ...
        Args:
            endpoint_url: URL de LocalStack (default: leído de config centralizada / AWS_ENDPOINT_URL en .env)
        ...
        if USE_CONFIG:
            ...
            self.endpoint_url = aws_config.get('endpoint_url')
            ...
            self.s3 = boto3.client('s3', **_shared_kwargs)
            self.dynamodb = boto3.client('dynamodb', **_shared_kwargs)
