...
try:
    from app.core.config import get_redis_config, get_aws_config
    USE_CONFIG = True
except ImportError:
    USE_CONFIG = False
...
class SystemHealthMonitor:
    ...
    def _init_aws_clients(self):
        ...
        # Health checks need to fail faster than the app-wide default timeouts
        _fast_cfg = BotocoreConfig(connect_timeout=0.5, read_timeout=1, retries={'max_attempts': 0})

        if USE_CONFIG:
            aws_config = get_aws_config()
            if not self.use_localstack:
                aws_config.pop('endpoint_url', None)
            aws_config['config'] = _fast_cfg
        elif self.use_localstack:
            aws_config = {
