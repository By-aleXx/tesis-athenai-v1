...
_BASE_DIR = Path(__file__).resolve().parent.parent.parent  # app/ml -> app -> athenai-dashboard (donde vive models/)

# Importar FeatureEngineer local
from app.ml.feature_engineering import FeatureEngineer

# Chequeo de patrones compartido con ThreatDetector (SQLi/XSS/CMDi normalizado)
from app.security.threat_detector import looks_malicious
...
try:
    from app.ml.continuous_learning_engine import get_continuous_learner
    from app.ml.drift_detector import get_drift_detector
    from app.ml.poisoning_detector import get_poisoning_detector
...
class AIEngine:
    ...
    def predict(self, payload: Optional[str]) -> Tuple[str, float]:
        ...
        try:
            ...
            # Patrones de ataque obvios (blacklist), delegado al detector compartido
            # de app.security.threat_detector — normaliza (URL/Unicode/HTML decode,
            # comentarios SQL) antes de aplicar las regexes, así que no se evade con
            # encoding simple como sí ocurría con el match de substring plano anterior.
            has_attack_pattern = looks_malicious(payload) is not None
