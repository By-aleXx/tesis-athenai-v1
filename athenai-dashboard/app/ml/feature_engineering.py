...
NOTA: existe una copia de este archivo en training/feature_engineering.py (usada por
el pipeline de entrenamiento standalone). Esta copia vive dentro del paquete `app` para
uso en runtime por el backend Flask y agrega el shim de compatibilidad de pickle al
final del archivo. Cualquier cambio a la lógica de extracción de features debe
aplicarse en AMBOS archivos.
"""
...
# Compat: modelos .pkl entrenados antes de mover este archivo a app/ml/
# guardaron la ruta de módulo plana 'feature_engineering'. Registrar un alias
# en sys.modules permite que joblib/pickle sigan resolviendo la clase.
sys.modules.setdefault('feature_engineering', sys.modules[__name__])
