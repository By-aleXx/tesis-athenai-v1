...
# lambda_function_hybrid.py y su import 'models/*.pkl' asumen CWD == lambda/,
# así que apuntamos sys.path ahí y hacemos chdir para preservar ese contrato
# ('lambda' es palabra reservada en Python, no se puede importar como paquete).
...
def main():
    ...
    models_to_copy = [
        (os.path.join(_REPO_ROOT, 'training/models/xgboost.pkl'), 'models/xgboost.pkl'),
        (os.path.join(_REPO_ROOT, 'training/models/feature_engineer.pkl'), 'models/feature_engineer.pkl'),
        (os.path.join(_REPO_ROOT, 'training/models/isolation_forest.pkl'), 'models/isolation_forest.pkl'),
        (os.path.join(_REPO_ROOT, 'training/models/auth_scaler.pkl'), 'models/auth_scaler.pkl'),
    ]
