...
# Ejecutar siempre desde lambda/ (donde viven lambda_function.py y dist/)
cd "$(dirname "$0")/../lambda"
...
zip -q dist/function.zip lambda_function.py

# Verificar que se creó
if [ -f "dist/function.zip" ]; then
    SIZE=$(du -h dist/function.zip | cut -f1)
    echo -e "${GREEN}✓ Paquete creado: dist/function.zip (${SIZE})${NC}"
else
