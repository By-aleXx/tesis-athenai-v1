...
# Ejecutar siempre desde lambda/ (donde vive lambda_function_hybrid.py)
cd "$(dirname "$0")/../lambda"
...
mkdir -p dist
rm -f dist/lambda_deployment_lite.zip
...
zip -r -q ../dist/lambda_deployment_lite.zip .
cd ..

ZIP_SIZE=$(du -h dist/lambda_deployment_lite.zip | cut -f1)
echo "  ✓ dist/lambda_deployment_lite.zip creado ($ZIP_SIZE)"

# Verificar tamaño
ZIP_BYTES=$(stat -f%z dist/lambda_deployment_lite.zip 2>/dev/null || stat -c%s dist/lambda_deployment_lite.zip)
...
awslocal lambda create-function \
    ...
    --zip-file fileb://dist/lambda_deployment_lite.zip \
    --timeout 60 \
    --memory-size 1024 \
    --region $REGION \
