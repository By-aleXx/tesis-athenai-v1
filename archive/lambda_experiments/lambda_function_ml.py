"""
AthenAI - Función Lambda con Isolation Forest
Versión híbrida con ML real para detección de anomalías de autenticación

Modelos:
- Isolation Forest: Detección de anomalías en autenticación
- Pattern-based: Detección de SQLi/XSS (temporal hasta DistilBERT)
"""
...
def load_isolation_forest():
    """Carga Isolation Forest y scaler (lazy loading)"""
    global isolation_forest_model, auth_scaler
...
def extract_auth_features(log_data: Dict) -> np.ndarray:
    """
    Extrae features para Isolation Forest
    
    Features (8 dimensiones):
    1. time_since_last_login (segundos)
    2. failed_attempts_count
    3. login_hour (0-23)
    4. is_weekend (0/1)
    5. unusual_location (0/1)
    6. login_day_of_week (0-6)
    7. geo_distance_km
    8. session_duration_avg (segundos)
    """
    
    # Valores por defecto
    features = np.zeros(8)
    
    # Feature 1: time_since_last_login
    features[0] = log_data.get('time_since_last_login', 3600)  # Default: 1 hora
    
    # Feature 2: failed_attempts_count
    features[1] = log_data.get('failed_attempts_count', 0)
    
    # Feature 3: login_hour
    try:
        timestamp = log_data.get('timestamp', datetime.utcnow().isoformat())
        dt = datetime.fromisoformat(timestamp.replace('Z', '+00:00'))
        features[2] = dt.hour
        features[5] = dt.weekday()  # Feature 6
        features[3] = 1 if dt.weekday() >= 5 else 0  # Feature 4: is_weekend
    except:
        features[2] = 12  # Default: mediodía
        features[3] = 0
        features[5] = 0
    
    # Feature 5: unusual_location
    features[4] = 1 if log_data.get('unusual_location', False) else 0
    
    # Feature 7: geo_distance_km
    features[6] = log_data.get('geo_distance_km', 0)
    
    # Feature 8: session_duration_avg
    features[7] = log_data.get('session_duration_avg', 1800)  # Default: 30 min
...
def detect_sqli_pattern(text: str) -> Dict[str, Any]:
    """Detección de SQLi basada en patrones (temporal)"""
...
def lambda_handler(event: Dict, context: Any) -> Dict:
    """Handler principal"""
    print("="*80)
    print("AthenAI - Sistema Híbrido de Detección (ML)")
    print("="*80)
    ...
    try:
        for record in event['Records']:
            ...
            if log_type == 'WEB_TRAFFIC':
                ...
                prediction = detect_sqli_pattern(text)
